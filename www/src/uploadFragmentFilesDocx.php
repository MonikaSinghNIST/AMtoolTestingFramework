...
<link rel="stylesheet" type="text/css" href="../css/layout.css" media="screen" />
	...
	<script src="http://code.jquery.com/jquery-latest.js"></script>
        <script type="text/javascript">
           function GenerateDocxFragmentSequential()
			{
			jQuery(document).ready(function($){
                ...
                $.ajax({
					...
					url: "GenerateFragmentDocxFromBeginning.php", //Ajax Action url
                    data: data,
					processData: false,
					contentType: false,
					cache: false,
			...
			function GenerateDocxFragmentRandom()
			{
			jQuery(document).ready(function($){
                ...
                $.ajax({
                    type: "POST", // Method type GET/POST    
					enctype: 'multipart/form-data',
					url: "GenerateFragmentDocxRandom.php", //Ajax Action url
                    data: data,
					processData: false,
					contentType: false,
					cache: false,
	...
	function redirect(){
	 ...
	 document.getElementById('frm').action = 'GenerateFragmentDocxFromBeginning.php';
	/*
 ...
 <div class="se-pre-con"></div>
	...
	<div id="mainWrapper">
		...
		<div id="subContentContainer">
    	  ...
    	  <div id="rightContent">
    	    <div id="breadcrumb">Fragment Identification ➤ <a href="GenerateFragment.php">Generate the Dataset</a> ➤ File type : <?php if($_GET['setType']=="TEXT"){ ?>Text<?php }else{ ?>Docx<?php } ?> ➤ Fragment type : <?php if($_GET['FragmentType']=="FromBeginning"){ ?>Sequential<?php }else{ ?>Random<?php } ?></div> 
		
        <form id="frm" method="post" <?php if(($_GET['setType']=="DOCX") && ($_GET['FragmentType']=="FromBeginning")){ ?> action="javascript:GenerateDocxFragmentSequential();" <?php }elseif(($_GET['setType']=="DOCX") && ($_GET['FragmentType']=="Random")){ ?> action="javascript:GenerateDocxFragmentRandom();" <?php }elseif(($_GET['setType']=="DOCX") && ($_GET['FragmentType']=="FromBeginning")){ ?> action="javascript:editXMLDoc();" <?php }elseif(($_GET['setType']=="DOCX") && ($_GET['FragmentType']=="Random")){ ?> action="javascript:editXMLDoc();" <?php } ?> enctype="multipart/form-data">
				  <table>
					 
                      <?php
					  //echo $_GET['setType']."---".$_GET['FragmentType'];
					  ?>
					  <tr><td width="50%">Enter the folder containing Docx files</td><td align="right" style="text-align: right"> <div align="right"><input accept=".docx,.DOCX" type="file" required="" name="docxFiles[]" id="docxFiles" multiple="" ></div></td></tr> 
