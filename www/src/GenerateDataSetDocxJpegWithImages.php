...
//DataUserGenerated\EmbeddedObjects\Docx\JPEG\Embedded_Objects
	...
	$delJpegfiles = glob('..\DataUserGenerated/EmbeddedObjects/Docx/JPEG/Embedded_Objects/*'); // get all file names
foreach($delJpegfiles as $delJpegfiles){ // iterate files
  if(is_file($delJpegfiles))
    unlink($delJpegfiles); // delete file
}

//DataUserGenerated\EmbeddedObjects\Docx\JPEG\Embedded_Files
	$delRsltfiles = glob('..\DataUserGenerated/EmbeddedObjects/Docx/JPEG/Embedded_Files/*'); // get all file names
foreach($delRsltfiles as $delRsltfiles){ // iterate files
  if(is_file($delRsltfiles))
    unlink($delRsltfiles); // delete file
}
	
	$delRsltfiles = glob('..\DataUserGenerated/EmbeddedObjects/Docx/JPEG/Original_Files/*'); // get all file names
foreach($delRsltfiles as $delRsltfiles){ // iterate files
  if(is_file($delRsltfiles))
    unlink($delRsltfiles); // delete file
}	
...
//////////////////////////////////Move JPEG to userJPEGFolder/////////////////////////////////////////////////	
		...
		for($i=0; $i<$total; $i++) {
		  ...
		  if ($tmpFilePath != ""){
			//Setup our new file path
			$newFilePath = "..\DataUserGenerated/EmbeddedObjects/Docx/JPEG/Embedded_Objects/" . $_FILES['jpeg']['name'][$i];
...
?>
 ...
 <?php  //echo exec('javac -cp "javasrc/jar/docx4j-2.8.0-sources.jar;javasrc/jar/poi-3.16.jar;javasrc/jar/poi-ooxml-3.16.jar;javasrc/jar/poi-ooxml-3.16.jar;javasrc/jar/poi-ooxml-schemas-3.16.jar;javasrc/jar/xmlbeans-2.6.0.jar;javasrc/jar/zip4j_1.3.2.jar" javasrc/DocxCopyJpegPHP.java'); 
		echo exec('java -cp "javasrc/jar/docx4j-2.8.0-sources.jar;javasrc/jar/poi-3.16.jar;javasrc/jar/poi-ooxml-3.16.jar;javasrc/jar/poi-ooxml-3.16.jar;javasrc/jar/poi-ooxml-schemas-3.16.jar;javasrc/jar/xmlbeans-2.6.0.jar;javasrc/jar/zip4j_1.3.2.jar;." javasrc.DocxCopyJpegPHP');
	...
	//$path = 'D:\\Pictures\\'; 
//exec("explorer '" . $path . "'");
	$fi1_1 = new FilesystemIterator("..\DataUserGenerated\EmbeddedObjects\Docx\JPEG\Embedded_Files", FilesystemIterator::SKIP_DOTS);
$numOfFile=iterator_count($fi1_1);
	...
	?>
					  ...
					  <tr><td colspan="2"><div id="breadcrumb" align="center"><?php echo $numOfFile; ?> embedded fils has been generated.&nbsp;&nbsp;<a href="..\DataUserGenerated//EmbeddedObjects//Docx//JPEG//Embedded_Files//" target="_blank" >Find here</a></div></td></tr>
