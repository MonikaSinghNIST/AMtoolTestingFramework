...
<script type="text/javascript">
		 ...
		 var ssdeepJpegACC="<?php echo round($_GET['ssdeepJpegACC'],2); ?>";
		 var ssdeepBmpACC="<?php echo round($_GET['ssdeepBmpACC'],2); ?>";
		 var ssdeepGifACC="<?php echo round($_GET['ssdeepGifACC'],2); ?>";
		 var ssdeepTiffACC="<?php echo round($_GET['ssdeepTiffACC'],2); ?>";
		 var sdhashJpegACC="<?php echo round($_GET['sdhashJpegACC'],2); ?>";
		 var sdhashBmpACC="<?php echo round($_GET['sdhashBmpACC'],2); ?>";
		 var sdhashGifACC="<?php echo round($_GET['sdhashGifACC'],2); ?>";
		 var sdhashTiffACC="<?php echo round($_GET['sdhashTiffACC'],2); ?>";
      ...
      function drawChart() {
        /*var data = google.visualization.arrayToDataTable([
		  ...
		  var data = google.visualization.arrayToDataTable([
          ['Accuracy (DOCX)', 'ssdeep', 'sdhash', 'mrsh'],
          ['JPEG', Number(ssdeepJpegACC), Number(sdhashJpegACC), 0],
          ['BMP', Number(ssdeepBmpACC), Number(sdhashBmpACC), 0],
          ['GIF', Number(ssdeepGifACC), Number(sdhashGifACC), 0],
          ['TIFF', Number(ssdeepTiffACC), Number(sdhashTiffACC), 0]
        ]);
        ...
        var chart = new google.charts.Bar(document.getElementById('ACC_chart'));
...
<script type="text/javascript">
		 ...
		 var ssdeepPPTXJpegACC="<?php echo round($_GET['ssdeepPPTXJpegACC'],2); ?>";
		 var ssdeepPPTXBmpACC="<?php echo round($_GET['ssdeepPPTXBmpACC'],2); ?>";
		 var ssdeepPPTXGifACC="<?php echo round($_GET['ssdeepPPTXGifACC'],2); ?>";
		 var ssdeepPPTXTiffACC="<?php echo round($_GET['ssdeepPPTXTiffACC'],2); ?>";
		 var sdhashPPTXJpegACC="<?php echo round($_GET['sdhashPPTXJpegACC'],2); ?>";
		 var sdhashPPTXBmpACC="<?php echo round($_GET['sdhashPPTXBmpACC'],2); ?>";
		 var sdhashPPTXGifACC="<?php echo round($_GET['sdhashPPTXGifACC'],2); ?>";
		 var sdhashPPTXTiffACC="<?php echo round($_GET['sdhashPPTXTiffACC'],2); ?>";
      ...
      function drawChart() {
        /*var data = google.visualization.arrayToDataTable([
		  ...
		  var data = google.visualization.arrayToDataTable([
          ['Accuracy (PPTX)', 'ssdeep', 'sdhash', 'mrsh'],
          ['JPEG', Number(ssdeepPPTXJpegACC), Number(sdhashPPTXJpegACC), 0],
          ['BMP', Number(ssdeepPPTXBmpACC), Number(sdhashPPTXBmpACC), 0],
          ['GIF', Number(ssdeepPPTXGifACC), Number(sdhashPPTXGifACC), 0],
          ['TIFF', Number(ssdeepPPTXTiffACC), Number(sdhashPPTXTiffACC), 0]
        ]);
        ...
        var chart = new google.charts.Bar(document.getElementById('ACC_chart_pptx'));
...
<body id="menu1"> 
    	  ...
    	  <div id="">
    	    <div id="breadcrumb"><div id="breadcrumb">Embedded Object Identification ➤ <a href="evaluateEmbeddedObject.php">Test Cases</a> ➤ Embedded Object Detection  ➤ <a href="result_report_embedded_object_based_search.php?threshold=22">Comparative Assessment</a></div> </div>
			  <form id="frm" method="get" action="javascript:redirect();">
                 ...
                 <?php $sn=0; ?>
                 <tr style="font-family: Arial, 'Century Gothic'; background-color: #0e375d; color:white; text-align: center;align-content: center;align-items: center;width: 1000px;"><td colspan="2" style="background-color: #0e375d;color:white; font-size: 18px; height: 30px;text-align:left; vertical-align: middle;padding-top: 10px;padding-left: 10px;">Embedded Object Detection Test </td></tr>
                
				 <tr class=""><td style="width: 9%; text-align: justify;" >&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<?php echo ++$sn."." ?></td>
                 <td style="align-content: space-around;text-align: justify; vertical-align: text-top;vertical-align: top;padding: 10px 10px 10px 10px;">This report presents the results of the "Embedded Object Detection Test". This test aims to identify the ability of existing schemes to detect an object with a file.</td></tr>	 
                 ...
                 <td style="align-content: space-around;text-align: justify; vertical-align: text-top;vertical-align: top;padding: 10px 10px 10px 10px;">Test is performed on the data-set of:<br><br>
                   <strong>i.</strong> 
                   <?php //echo $_GET['numOfJpegDocx']+$_GET['numOfJpegDocxWithOutImg']; ?> 100 docx file with embedded with 10<?php //echo $_GET['numOfJpeg']; ?> Jpeg, 100 docx files embedded with  <?php //echo $_GET['numOfBmp']; ?>  10 Bmp, 100 docx files embedded with  <?php //echo $_GET['numOfTiff']; ?> 10 Tiff and <?php //echo $_GET['numOfGifDocx']+$_GET['numOfGifDocxWithOutImg']; ?>100 docx files embedded with  <?php //echo $_GET['numOfGif']; ?> 10 Gif images.
                 <br>
				...
				// $totalComparision=($_GET['numOfJpegDocx']*$_GET['numOfJpeg'])+($_GET['numOfJpegDocxWithOutImg']*$_GET['numOfJpeg'])+($_GET['numOfBmpDocx']*$_GET['numOfBmp'])+($_GET['numOfBmpDocxWithOutImg']*$_GET['numOfBmp'])+($_GET['numOfTiffDocx']*$_GET['numOfTiff'])+($_GET['numOfTiffDocxWithOutImg']*$_GET['numOfTiff'])+($_GET['numOfGifDocx']*$_GET['numOfGif'])+($_GET['numOfGifDocxWithOutImg']*$_GET['numOfGif']);
				 ...
				 ?>
                 <tr><td style="width: 9%; text-align: justify;" >&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<?php echo ++$sn."." ?>&nbsp;</td><td style="align-content: space-around;text-align: justify; vertical-align: text-top;vertical-align: top;padding: 10px 10px 10px 10px;">Total number of comparision performed by each scheme <br> <strong>i.</strong> for docx files: 4000<?php //echo $totalComparision; ?><br> 
                   <strong>ii.</strong> for pptx: 4000 </td></tr>
				 ...
				 if($ssdeepPRE>$sdhashPRE){
					 $avgPRE=$avgPRE."ssdeep are more precise than sdhash on an avg. for both DOCX and PPTX.";
				 }else{
				 	$avgPRE=$avgPRE."sdhash are more precise than ssdeep on an avg. for both DOCX and PPTX";
				...
				<tr><td colspan="2"><div id="RECALL_chart_pptx" style="width: 600px; height: 300px;margin-left:86px;"></div></td></tr> 
                 ...
                 <!--<tr style="text-align:justify;"><td style="width: 9%; text-align: justify;" >&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<?php //echo ++$sn."."; ?>&nbsp;</td><td style="align-content: space-around;text-align: justify;"><?php echo $avgRECALL; ?></td></tr>-->
				 ...
				 if($ssdeepFSCORE>$sdhashFSCORE){
                  ...
                  <tr style="text-align:justify;"><td style="width: 9%; text-align: justify;" >&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<?php echo ++$sn."." ?>&nbsp;</td><td style="align-content: space-around;text-align: justify;"><?php echo $avgFSCORE; ?> Shown in figure below.</td></tr>
    ...
    <tr align="right"><td  colspan="2" align="right" ><input type="submit" style="margin-left:50%" value="Back"  onClick="location.href = 'result_report_embedded_object_based_search.php?threshold=22';"></td></tr>
