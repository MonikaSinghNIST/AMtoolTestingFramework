...
<link rel="stylesheet" type="text/css" href="../css/layout.css" media="screen" />
	...
	<script src="http://code.jquery.com/jquery-latest.js"></script>
        <script type="text/javascript">
           function GenerateTextFragmentSequential()
			{
			jQuery(document).ready(function($){
                ...
                $.ajax({
					...
					url: "GenerateFragmentTextFromBeginning.php", //Ajax Action url
                    data: data,
					processData: false,
					contentType: false,
					cache: false,
			...
			function GenerateTextFragmentRandom()
			{
			jQuery(document).ready(function($){
                ...
                $.ajax({
                    type: "POST", // Method type GET/POST    
					enctype: 'multipart/form-data',
					url: "GenerateFragmentTextRandom.php", //Ajax Action url
                    data: data,
					processData: false,
					contentType: false,
					cache: false,
	...
	function redirect(){
	 ...
	 document.getElementById('frm').action = 'GenerateFragmentTextFromBeginning.php';
	/*
		...
		<div id="subContentContainer">
    	  ...
    	  <div id="rightContent">
    	    <div id="breadcrumb">Fragment Identification ➤ <a href="GenerateFragment.php">Generate the Dataset</a> ➤ File type : <?php if($_GET['setType']=="TEXT"){ ?>Text<?php }else{ ?>Docx<?php } ?> ➤ Fragment type : <?php if($_GET['FragmentType']=="FromBeginning"){ ?>Sequential<?php }else{ ?>Random<?php } ?></div> 
			  <form id="frm" method="post" <?php if(($_GET['setType']=="TEXT") && ($_GET['FragmentType']=="FromBeginning")){ ?> action="javascript:GenerateTextFragmentSequential();" <?php }elseif(($_GET['setType']=="TEXT") && ($_GET['FragmentType']=="Random")){ ?> action="javascript:GenerateTextFragmentRandom();" <?php }elseif(($_GET['setType']=="DOCX") && ($_GET['FragmentType']=="FromBeginning")){ ?> action="javascript:editXMLDoc();" <?php }elseif(($_GET['setType']=="DOCX") && ($_GET['FragmentType']=="Random")){ ?> action="javascript:editXMLDoc();" <?php } ?> enctype="multipart/form-data">
				  <table>
					 
                      <?php
					  //echo $_GET['setType']."---".$_GET['FragmentType'];
					  ?>
					  <tr><td width="50%">Enter the folder containing Text files</td><td align="right" style="text-align: right"> <div align="right"><input accept=".text,.txt" type="file" required="" name="textFiles[]" id="textFiles" multiple="" ></div></td></tr> 
