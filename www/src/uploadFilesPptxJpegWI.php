    	    ...
    	    <div id="breadcrumb">Embedded Object Identification ➤ <a href="GenerateEmbeddedObject.php">Generate the Dataset</a> ➤ File type : PPTX ➤ Embedded object type : <?php echo $objct; ?></div> 
			  <form id="frm" method="post" action="javascript:editXMLDoc();" enctype="multipart/form-data">
				  <table>
                      ...
                      <tr><td width="50%">Enter the folder containing <?php if($objct=="JPEG"){ ?> JPEG <?php }else if($objct=="BMP"){ ?>BMP <?php }else if($objct=="TIFF"){ ?>TIFF <?php }else if($objct=="GIF"){ ?>GIF <?php } ?> images</td><td align="right" style="text-align: right"><div align="right"><input accept="image/jpeg" required="" type="file" name="jpeg[]" id="jpeg" multiple=""></div></td></tr> 
